...
func (tm *ClaimTxManager) monitor() {
...
func (tm *ClaimTxManager) getNextNonce(from common.Address, add bool) (uint64, error) {
...
func (tm *ClaimTxManager) addClaimTx(depositCount uint, from common.Address, to *common.Address, value *big.Int, data []byte, dbTx pgx.Tx) error {
	...
	// get next nonce
	nonce, err := tm.getNextNonce(from, true)
	if err != nil {
...
func (tm *ClaimTxManager) monitorTxs(ctx context.Context) error {
	...
	mTxs, err := tm.storage.GetClaimTxsByStatus(ctx, statusesFilter, dbTx, txLimit)
	if err != nil {
...
func (tm *ClaimTxManager) ReviewMonitoredTx(ctx context.Context, mTx *ctmtypes.MonitoredTx, reviewNonce bool) error {
	...
	if reviewNonce {
		// check nonce
		nonce, err := tm.getNextNonce(mTx.From, false)
		if err != nil {
